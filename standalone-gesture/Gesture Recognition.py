        ...
        while True:
            ...
            for (x, y, fw, fh) in faces:
                ...
                if face_recognizer and len(known_encodings) > 0:
                    ...
                    # threshold 70 = เข้มขึ้น — ถ้า confidence สูงกว่านี้ถือว่าไม่รู้จัก
                    if confidence < 70:
            ...
            if results.pose_landmarks:
                ...
                if is_blocked:
                    ...
                    if prediction is not None:
                        ...
                        log_movement_data(identified_name if identified_name else current_name, label_str, avg_features)

            # --- 3. แสดงผลหน้าจอ ---
            name_color = (0, 255, 0) if current_name != "Unknown" else (100, 100, 100)
            conf_text = f" (conf:{face_confidence:.0f})" if face_confidence is not None else ""
            cv2.putText(image, f"Name: {current_name}{conf_text}", (50, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.1, name_color, 3)
